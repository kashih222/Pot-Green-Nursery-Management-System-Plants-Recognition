...
const PlantCatalog = () => {
  ...
  const filteredPlants = plants
    ...
    .sort((a, b) => {
      if (sortBy === "price") {
        ...
        const aPrice = Math.min(a?.prices?.small || Infinity, a?.prices?.medium || Infinity, a?.prices?.large || Infinity);
        const bPrice = Math.min(b?.prices?.small || Infinity, b?.prices?.medium || Infinity, b?.prices?.large || Infinity);
  ...
  const addToCart = async (plant) => {
    ...
    try {
      console.log('Attempting to add to cart:', { plantId: plant._id, size: 'small' });
      ...
      {isLoadingPlants ? (
        ...
        <div className="container w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-36 mt-40">
          {paginatedPlants.map((plant) => (
            <div
              key={plant._id}
              className="popular__card bg-green-950 pt-20 rounded-md relative flex"
            >
              {plant?.plantImage && (
                <img
                  src={`http://localhost:8020${plant.plantImage.startsWith("/uploads") ? plant.plantImage : `/uploads/${plant.plantImage}`}`}
                  alt={plant.plantName}
              ...
              <div className="p-4 w-full">
                ...
                <p className="text-sm w-full overflow-hidden">
                ...
                <div className="flex items-center mb-2">
                  ...
                  <span className="flex text-yellow-500 text-xs py-3">
                    ({plant?.rating || 0})
                  </span>
                </div>
                <div className="w-full flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      {plant?.stockQuantity ? 
                        `${plant.stockQuantity.small + plant.stockQuantity.medium + plant.stockQuantity.large} in stock` : 
                        "Out of stock"}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button 
                      ...
                      disabled={loading || !plant?.stockQuantity || 
                        (plant.stockQuantity.small + plant.stockQuantity.medium + plant.stockQuantity.large) === 0}
                    ...
                    >
                      {wishlist.includes(plant._id) ? <IoHeart /> : <IoHeartOutline />}
                    </button>
                  </div>
                </div>
              </div>
        ...
        <DialogContent>
          {selectedPlant && (
            <FormControl fullWidth variant="outlined" className="mt-4">
              <InputLabel style={{ color: '#eab308' }} className="bg-green-950 px-2">Size</InputLabel>
              <Select
                value={selectedSize}
              ...
              >
                {['small', 'medium', 'large'].map((size) => (
                  ...
                  >
                    {size.charAt(0).toUpperCase() + size.slice(1)} - Rs. {selectedPlant.prices[size]}
                    {selectedPlant.stockQuantity[size] === 0 ? ' (Out of Stock)' : ` (${selectedPlant.stockQuantity[size]} available)`}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
